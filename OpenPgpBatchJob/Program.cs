...
namespace OpenPgpBatchJob
{
    class Program
    {
        ...
        private static string PrintPropertiesOfOpenPGPKey(Libgpgme.PgpKey key)
        {
            ...
            sb.AppendLine(ObjectPrinter.PrintProperties(key.Subkeys, subKeyPropertiesToPrint).ToString());

            if (key.Subkeys.Next != null)
            {
                sb.AppendLine("Subkey 2 Properties: ");
                sb.AppendLine(ObjectPrinter.PrintProperties(key.Subkeys.Next, subKeyPropertiesToPrint).ToString());
            }
